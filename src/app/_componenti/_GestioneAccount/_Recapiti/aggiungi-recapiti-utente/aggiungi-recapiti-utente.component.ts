...
@Component({
  selector: 'app-aggiungi-recapiti-utente', // Updated selector
  templateUrl: './aggiungi-recapiti-utente.component.html', // Updated template URL
  styleUrls: ['./aggiungi-recapiti-utente.component.scss'] // Updated style URL
})
export class AggiungiRecapitiComponent implements OnInit, OnDestroy {
  ...
  obsAddRecapito(dati: Partial<Recapito>) {
    return this.api.postRecapiti(dati).pipe( // Changed to postRecapiti
      takeUntil(this.distruggi$),
  ...
  resetForm() {
    ...
    this.newIdContatto = this.loggedInIdContatto; // Resetting to the logged-in idContatto
  }
